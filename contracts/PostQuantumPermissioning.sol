...
contract PostQuantumPermissioning {
  ...
  function transactionAllowed(address/*sender*/,
                              address target,
                              uint256/*value*/,
                              uint256/*gasPrice*/,
                              uint256/*gasLimit*/,
                              bytes calldata payload) external returns (bool) {
    if (target != relayHub) {
      return false;
    }
    ...
    (signingData, v, r, s, falconPublicKey, falconSignature) = abi.decode(payload[4:], (bytes, uint8, bytes32, bytes32, bytes, bytes));
